...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  ...
  8.5. **Expression（値の渡し方）の検証** ⚠️ 重要
       - 達成目標: すべてのExpressionが正しく評価されることを確認する
       - 具体例:
         - 前ノード参照: `{{ $node["ノード名"].json.field }}`が存在するノードを参照しているか
         - 配列アクセス: `{{ $json.items[0] }}`でitemsが配列か確認
         - 環境変数: `{{ $env.VAR }}`の変数が定義されているか確認
       - 確認事項:
         - **Expression構文エラー**: すべての`{{ }}`が正しく閉じられているか
         - **ノード名の一致**: `$node["ノード名"]`のノード名が実際に存在するか
         - **データ構造の一致**: 参照するフィールドが実際に存在するか
         - **型エラー防止**: 文字列/数値/配列の型が期待通りか
         - **validate_workflow_expressions実行**: MCPの式検証ツールを活用

  8.6. **複数ノードからのデータ取得パスの検証** ⚠️ 重要
       - 達成目標: 複数ノードのデータを参照するノードが、すべてのソースノードに到達できるか確認
       - 具体例:
         - Code Nodeが`$node["A"]`、`$node["B"]`、`$node["C"]`を参照する場合、A/B/Cすべてに実行パスが存在するか
         - MergeノードにすべてのBranchが接続されているか
       - 確認事項:
         - **参照ノードの到達性**: `$node["X"]`で参照されるノードXが、参照元ノードの実行前に必ず実行されるか
         - **並列実行の同期**: 並列実行される複数ノードを参照する場合、Mergeノードで同期されているか
         - **データ利用可能性**: 参照時点でデータが確実に存在するか（実行順序の確認）
         - **循環参照の回避**: ノードAがBを参照、BがAを参照する循環が存在しないか
         - **情報の網羅性**: 1つのノードだけでは情報が不足する場合、必要なすべてのノードからデータを取得しているか

...
- 処理詳細手順:
  ...
  9. 各グループに Sticky Note を追加し、各ノードに詳細なコメントを追加:
     ...
     - `_comment`フィールド: 各ノードの役割と処理概要を1-2行で記述
...
2. 配置位置: 関連ノードの背景（後ろ）に配置し、視覚的なグループ化を実現
...
### Sticky Note の内容構造
各Sticky Noteには必ず以下を含める:
- 目的: このグループが何のために存在するのか
- 背景: なぜこの処理が必要なのか、どういう状況で使われるのか
- 処理の流れ: 1, 2, 3のステップで具体的な処理を記述
- 達成したいこと: 最終的なゴール、ユーザーにとっての価値
- 次のステップ: 次にどのグループへ処理が移るのか
...
- 処理詳細手順:
  ...
  7. すべてのノード間接続を明示的に定義
  8. メインワークフローと Error Workflow を紐付け
  9. 接続の完全性を確認
...
各Sticky Noteには必ず以下を含める:
- 目的: このグループが何のために存在するのか
- 背景: なぜエラーハンドリングが重要なのか
- 処理の流れ: エラー発生から通知までのステップ
- 達成したいこと: エラー対応時間の短縮、迅速な問題解決
- 重要性: 本番環境での必要性を強調

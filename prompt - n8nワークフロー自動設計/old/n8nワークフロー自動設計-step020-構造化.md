...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  - step010で収集した業務要件を、n8nの8層フレームワーク（Trigger/Fetch/Validate/Transform/Conditional/Execute/Integrate/Output）に分解する
  - 各層に必要なタスクを特定し、タスク間の依存関係を明確にする
  - 並列実行可能な箇所を特定し、ワークフローの効率性を最大化する設計を行う
  - 次ステップ（タスク分解）で10-50ノードに分解できる粒度まで構造を明確化する

  1. **トリガー層の判定**
     - 達成目標: ワークフロー起動の種類と設定を決定する
     - 具体例: Schedule Trigger（毎日9時）、Google Drive Trigger（ファイルアップロード時）、Webhook、Manual Trigger
     - 確認事項: トリガー頻度、データ取得方法、エラー時の再実行ポリシー

  2. **データ取得箇所の特定（並列取得の可能性を検討）**
     - 達成目標: データソースからのデータ取得タスクを特定し、並列化可能性を判断する
     - 具体例: Google Driveからファイル取得、Notionからページ取得、複数APIの並列呼び出し
     - 確認事項: 取得データの依存関係、並列実行によるパフォーマンス向上、API制限

  3. **データ検証の必要性判定**
     - 達成目標: 取得データの妥当性チェックが必要な箇所を特定する
     - 具体例: ファイル形式チェック、必須フィールド存在確認、データ範囲検証
     - 確認事項: 検証ルール、エラー時の処理（スキップ/停止/通知）

  4. **データ変換・加工の内容特定**
     - 達成目標: データ変換が必要な箇所とその処理内容を明確にする
     - 具体例: JSON整形、テキスト抽出、日時フォーマット変換、AI処理（音声→テキスト）
     - 確認事項: 変換ロジックの複雑度、AI Agent Nodeの使用箇所、Code Nodeの必要性

  5. **条件分岐の特定**
     - 達成目標: 処理フローの分岐条件とそれぞれのパスを定義する
     - 具体例: ファイル種別による分岐、データ量による処理方法の切り替え、エラー判定
     - 確認事項: 分岐条件、各パスの処理内容、デフォルト処理

  6. **実行アクションの列挙**
     - 達成目標: データ処理後に実行すべきアクションを全て洗い出す
     - 具体例: Notion書き込み、Discord通知、ファイル保存、データベース更新
     - 確認事項: アクションの実行順序、並列実行の可能性、エラーハンドリング

  7. **データ統合の必要性判定**
     - 達成目標: 複数データソースからのデータを統合する箇所を特定する
     - 具体例: 並列取得したデータのマージ、複数API結果の結合、集計処理
     - 確認事項: 統合方法（マージ/結合/集計）、データ形式の統一

  8. **出力・記録先の特定**
     - 達成目標: 処理結果の出力先と出力形式を明確にする
     - 具体例: Notionデータベース書き込み、Discordメッセージ送信、ログファイル保存
     - 確認事項: 出力形式、複数出力先、エラー時の通知先
  9. n8n-MCP 実行（テンプレート使用の場合）:

     **実行基準**:
     - **実行タイミング**: 8層構造への分解が完了し、step010で発見したテンプレートを活用する場合
     - **実行条件**:
       - step010でテンプレートが発見されている
       - テンプレートの構造が現在の業務要件にマッチする可能性が高い
       - テンプレートの詳細構造を理解する必要がある
     - **実行内容**:
       ```
       順次実行:
       - get_template_workflow({templateId: "{{step010で発見したテンプレートID}}"})
       - get_node_documentation({nodeType: "{{テンプレート内の主要ノードタイプ}}"})
       ```
     - **判断ポイント**:
       - テンプレート活用: step010で有用なテンプレートが見つかっている
       - 構造の一致度: テンプレートのフローが8層構造にマッピング可能
       - ノード理解: テンプレート内の特定ノードの詳細理解が必要
     - **スキップ条件**:
       - step010でテンプレートが見つからなかった場合
       - テンプレートを使用せず完全カスタム設計を行う場合
       - 業務要件がテンプレートと大きく異なる場合

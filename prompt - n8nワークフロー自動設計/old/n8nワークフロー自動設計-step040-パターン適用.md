...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  - step030で分解したノード群に、Sequential/Parallel/Loop/Conditionalの4つの実行パターンを適用する
  - 全ノード間の接続マトリックス（main出力/error出力）を完全に定義する
  - クリティカルパスを特定し、ボトルネックとなる箇所を明確にする
  - レート制限対策とバッチサイズを設定し、API制限を回避する設計にする
  - 次ステップ（n8n設計変換）で具体的なn8nノード設定ができる粒度まで接続を明確化する

  1. **並列実行の特定とマージ戦略決定**
     - 達成目標: 依存関係のないタスクを並列実行グループとして定義し、マージ方法を決定する
     - 具体例: 複数API呼び出し→並列実行→Mergeノードで統合、複数通知→並列送信→NoOpで完了
     - 確認事項: 並列実行するタスクのリスト、マージタイミング、マージ方法（全て待機/最初の1つ/N個）

  2. **ループ処理（大エリア・小エリアの両方）の特定、バッチサイズ・レート制限設定**
     - 達成目標: ループが必要な箇所を特定し、バッチサイズとレート制限を設定する
     - 具体例:
       - 大エリアループ: 1000件データ→Split in Batches（100件/バッチ）→処理→Loop Back
       - 小エリアループ: 配列内ループ→各要素処理→次要素
       - レート制限: API制限（100req/min）→Wait 600ms設定
     - 確認事項: ループ範囲、バッチサイズ、レート制限値、終了条件、エラー時の再試行

  3. **条件分岐の特定と全パス定義**
     - 達成目標: すべての条件分岐を特定し、各パスの処理内容とデフォルト処理を定義する
     - 具体例: ファイル形式分岐（M4A→音声処理/PDF→テキスト抽出/その他→エラー）
     - 確認事項: 分岐条件の完全性、すべてのケースがカバーされているか、デフォルト処理

  4. **全ノード間の接続マトリックス作成（main/error 出力）**
     - 達成目標: すべてのノード間の接続を明示的に定義し、孤立ノードが0個であることを保証する
     - 具体例: ノードA main→ノードB、ノードA error→Error Handler、ノードB main→ノードC
     - 確認事項: すべてのノードのmain出力先、error出力先、到達可能性

  5. **クリティカルパスとボトルネックの特定**
     - 達成目標: ワークフロー実行時間に最も影響するパス（クリティカルパス）を特定する
     - 具体例: 音声文字起こし（5分）→クリティカルパス、Discord通知（1秒）→非クリティカル
     - 確認事項: 各ノードの処理時間、依存関係、並列化可能性、最適化余地

  6. n8n-MCP 実行（必要に応じて）:

     **実行基準**:
     - **実行タイミング**: 実行パターン適用が完了し、複雑なノード（Merge、Split in Batches等）の設定を確認する段階
     - **実行条件**:
       - Mergeノード、Split in Batches、Loop Over Itemsの詳細設定が不明な場合
       - 複雑な条件分岐（Switch、Filter）の実装パターンを確認する必要がある場合
       - レート制限やバッチ処理のベストプラクティスを参照したい場合
     - **実行内容**:
       ```
       並列実行:
       - get_node_documentation({nodeType: "n8n-nodes-base.merge"})
       - get_node_documentation({nodeType: "n8n-nodes-base.splitInBatches"})
       - search_node_properties({nodeType: "n8n-nodes-base.switch", query: "条件分岐"})
       - search_templates({query: "バッチ処理 レート制限"})
       ```
     - **判断ポイント**:
       - Merge設定: 並列タスクのマージ方法（Wait for all/First response）を確認
       - Split設定: バッチサイズ、リセット条件の推奨値を取得
       - レート制限: API制限に応じた適切なWait時間を設定
       - パターン参照: 類似ワークフローの実装パターンを確認
     - **スキップ条件**:
       - すべてのパターン適用が明確で、ノード設定も確定している場合
       - シンプルなSequentialフローのみで複雑なパターンが不要な場合

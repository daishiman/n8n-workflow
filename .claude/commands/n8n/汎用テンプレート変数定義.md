...
| 変数名 | デフォルト値 | 説明 |
|--------|-------------|------|
| `{{OPENAI_MODEL}}` | `"gpt-4o"` | モデル名（gpt-4o, gpt-4o-mini, gpt-4.5-turbo） |
...
| 変数名 | デフォルト値 | 説明 |
|--------|-------------|------|
| `{{ANTHROPIC_MODEL}}` | `"claude-4-5-sonnet-20250929"` | モデル名 |
...
| 変数名 | デフォルト値 | 説明 |
|--------|-------------|------|
| `{{GEMINI_MODEL}}` | `"gemini-2.5-flash-exp"` | モデル名 |
...
| 変数名 | デフォルト値 | 必須 | 説明 |
|--------|-------------|------|------|
| `{{STICKY_NOTE_CONTENT}}` | `"## Group ..."` | ❌ | Markdown形式のコンテンツ（必ず `nodes` ブロックを含める） |
| `{{STICKY_NOTE_HEIGHT}}` | `420` | ✅ | 高さ（px）—`calculateStickyDimensions()`で算出し最小値420以上 |
| `{{STICKY_NOTE_WIDTH}}` | `520` | ✅ | 幅（px）—`calculateStickyDimensions()`で算出し最小値520以上 |
| `{{STICKY_NOTE_COLOR}}` | `6` | ✅ | 色番号（1-7のみ使用、color=0/1は禁止。全体=7/メイン=6/エラー=5） |

**Sticky Note 色番号（color=0/1は使用禁止）**:
- `2`: 薄緑色
- `3`: 薄青色
- `4`: 薄紫色
- `5`: 薄赤色（エラーフロー推奨）
...
// ✅ 有効
validate_node_minimal("nodes-base.stickyNote", {"height": 420, "width": 520, "color": 6})
→ {"valid": true, "missingRequiredFields": []}
```

**ノードブロック必須**:
- `{{STICKY_NOTE_CONTENT}}` には必ず ```nodes``` ブロックを含め、`- ノード表示名 (ノード種別 / Node ID)` 形式で列挙する。
- グループ外のノードは記載しない。エラーフローの場合は `nodes-error` ブロックを追加してもよい。

**calculateStickyDimensionsヘルパーの推奨**:

```javascript
function calculateStickyDimensions(content, { minHeight = 420, minWidth = 520 }) {
  const lines = content.split('\n');
  const nodeBlocks = [...content.matchAll(/```nodes([\s\S]*?)```/g)];
  const nodeCount = nodeBlocks
    .map(block => block[1].split('\n').filter(line => line.trim().startsWith('- ')).length)
    .reduce((sum, count) => sum + count, 0);

  const baseHeight = 28 * lines.length + 18 * nodeCount + 40;
  const baseWidth = Math.max(minWidth, Math.max(...lines.map(line => line.length)) > 70 ? minWidth + 80 : minWidth);

  const height = Math.ceil(Math.max(minHeight, baseHeight) / 10) * 10;
  const width = Math.ceil(baseWidth / 10) * 10;
  return { height, width };
}
```

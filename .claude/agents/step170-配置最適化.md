...
- 適用: シーケンシャルな処理フロー
- 例: 画像上部の「Company Intelligence & Research Pipeline」

**パターンB: 縦一列配置**（推奨: AI処理グループ）
```
┌─────────────────┐
│ Sticky Note:    │
│ Group 2         │
...
└─────────────────┘
```
- ノード間隔: 水平0px（同一X座標）、垂直180px
- 適用: AI Agent + サブノード（Language Model, Memory, Tools）
- 例: 画像の各グループ（Gamma API, Investor Outreach等）

**パターンC: グリッド配置**（推奨: 複雑なグループ）
```
┌───────────────────────────────┐
│ Sticky Note: Group 3          │
│  ┌────┐   ┌────┐              │
...
グループ間隔: 500px（Sticky Note幅 + マージン）
```
...
| グループID | 整列パターン | 基準座標(X, Y) | Sticky Note座標 | 色 |
|-----------|------------|----------------|----------------|-----|
| 全体フロー | 横一列 | (100, 100) | (100, 50) | 7（薄ピンク） |
| G001 | 縦一列 | (100, 400) | (50, 350) | 6（薄オレンジ） |
| G002 | 縦一列 | (600, 400) | (550, 350) | 3（薄青） |
| G003 | 縦一列 | (100, 900) | (50, 850) | 4（薄紫） |
| G004 | 縦一列 | (600, 900) | (550, 850) | 2（薄緑） |
| E001 | 縦一列 | (1100, 400) | (1050, 350) | 5（薄赤） |

4. **視覚的整合性チェック**
- グループ間の最小間隔: 500px（Sticky Note包含範囲を考慮）
- グループ内ノード間隔: パターンAは水平300px、パターンBは垂直180px
...
| G001 | 縦一列 | (100, 400) | (50, 350) | 6 | 水平0px, 垂直300px |
| G002 | 縦一列 | (600, 400) | (550, 350) | 3 | 水平500px, 垂直0px |
| G003 | 縦一列 | (100, 900) | (50, 850) | 4 | 水平-500px, 垂直500px |

### 整列パターン適用数
- 横一列配置: 1グループ（全体フロー）
- 縦一列配置: 4グループ（G001-G004）
- グリッド配置: 0グループ

### 視覚的整合性
- 最小グループ間隔: 500px ✅
- Sticky Note包含確認: すべてのグループで確認済み ✅
- 色の多様性: 5色使用（7, 6, 3, 4, 5） ✅
```
...
3. **配置戦略決定**（ステップ0のグループ整列配置戦略に基づく）
   - メインフローの配置方針: レイヤーベース垂直配置 + グループ整列パターン適用
   - エラーフローの配置方針: 右側または下部に分離配置 + 縦一列整列
   - 並列グループの配置方針: 水平方向に展開 + グループ間隔500px確保
...
- 配置方針: レイヤー垂直配置 + グループ整列パターン（縦一列）
- グループ整列: 各グループ内のノードは垂直180px間隔で整列

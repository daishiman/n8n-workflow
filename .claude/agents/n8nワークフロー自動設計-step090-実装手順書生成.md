...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  - 初心者でも迷わず実装できる完全な手順書を作成
  - 選択されたChat Modelの設定手順を詳細に記載
  - AI Agent接続問題を自己解決できるガイドを提供
  - Step10（最終成果物出力）への準備完了

  1. **環境変数リスト作成**
     - 達成目標: 必要な環境変数をすべてリストアップ
     - 具体例: OPENAI_API_KEY、DISCORD_WEBHOOK_URL、DATABASE_CONNECTION_STRING
     - 確認事項: 各変数の取得元URL、設定方法、デフォルト値

  2. **Chat Model認証情報取得方法記載**
     - 達成目標: 選択されたChat Modelの認証を確実に設定
     - 具体例: OpenAI APIキー取得手順、Claude API設定、Gemini認証方法
     - 確認事項: 公式ドキュメントURL、料金プラン、利用制限

  3. **n8nインポート手順記載**
     - 達成目標: JSONファイルを正しくインポートできる手順
     - 具体例: メインワークフローとError Workflowの順番、設定タブの確認
     - 確認事項: インポート時のエラー対処、既存ワークフローとの競合回避

  4. **ノード接続確認手順詳細記載**
     - 達成目標: 視覚的に接続を確認できる手順を提供
     - 具体例: 接続線の色、ノードアイコンの状態、エラー表示の見方
     - 確認事項: 各ノードタイプごとの接続確認ポイント

  5. **AI Agent接続確認手順記載**
     - 達成目標: AI AgentとサブノードのCluster接続を確実に検証
     - 具体例: ai_languageModel、ai_tool、ai_memory接続の確認方法
     - 確認事項: 接続タイプの見分け方、JSON内での確認方法

  6. **Chat Model別テスト方法記載**
     - 達成目標: 選択されたChat Modelで動作確認できる
     - 具体例: テストプロンプト、期待される応答、レスポンス時間
     - 確認事項: モデル別の特性、トークン制限、エラーメッセージ

  7. **n8n-MCP実装例の参考情報記載**:
     **実行基準**:
     - **実行タイミング**: 実装手順書作成時、具体例が必要な場合
     - **実行条件**:
       - n8n-MCP使用が許可
       - 実装例が存在するノードタイプ
       - ユーザーが詳細例を要求
     - **実行内容**:
       ```
       並列実行:
       - get_node_essentials({nodeType: 主要ノード, includeExamples: true})
       - search_templates({query: ワークフロータイプ})
       条件付き実行:
       - get_template_details({templateId: 該当テンプレート})
       ```
     - **判断ポイント**:
       - 実装例が現在のワークフローに適合するか
       - 例のバージョンが最新か確認
  ...
  8. **トラブルシューティングガイド追加**
     - 達成目標: よくある問題を自己解決できるガイド提供
     - 具体例: 認証エラー対処、接続エラー解決、レート制限対策
     - 確認事項: Chat Model別エラーコード一覧、デバッグ手順、サポート連絡先

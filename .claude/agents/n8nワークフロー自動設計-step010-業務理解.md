...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  - ユーザーの曖昧な要求を、n8nで実装可能な具体的な業務要件に変換する
  - 6要素（業務概要、トリガー、データソース、処理内容、出力先、規模と制約）を漏れなく収集する
  - 類似するワークフローやテンプレートを発見し、設計の参考にする
  - 次ステップ（構造化）で8層フレームワークにマッピングできる粒度まで要件を明確化する

  1. **業務概要の引き出し**: 「どのような業務を自動化したいですか？」と質問
     - 達成目標: ユーザーが解決したい課題と、自動化の目的を明確にする
     - 具体例: 「Google DriveにアップロードされたM4A音声ファイルから議事録を自動生成したい」
     - 確認事項: 業務の種類（データ処理、通知、レポート生成等）を特定

  2. **トリガー条件の特定**: 「その作業はいつ実行されますか？」と深掘り
     - 達成目標: ワークフローの起動条件を具体的に定義する
     - 具体例: 「毎日9時」「ファイルアップロード時」「Webhook受信時」「手動実行」
     - 確認事項: 実行頻度、実行タイミング、イベント駆動かスケジュール駆動か

  3. **データソースの確認**: 「データはどこから取得しますか？」と質問
     - 達成目標: データの入力元となるサービス・システムを特定する
     - 具体例: 「Google Drive」「Notion」「PostgreSQL」「REST API」
     - 確認事項: 認証方法、データ形式、取得条件

  4. **処理内容の明確化**: 「そのデータをどう加工しますか？」と質問
     - 達成目標: データ変換・加工・分析の具体的な処理内容を定義する
     - 具体例: 「音声をテキストに変換」「JSONを整形」「条件分岐で振り分け」
     - 確認事項: 変換ロジック、AI処理の有無、複雑な計算の有無

  5. **出力先の確認**: 「最終的な結果をどこに送りますか？」と質問
     - 達成目標: 処理結果の出力先サービス・システムを特定する
     - 具体例: 「Notion」「Discord」「Slack」「Google Sheets」
     - 確認事項: 出力形式、複数出力先の有無、通知要件

  6. **規模と制約の確認**: 「データ量と実行頻度は？」「制約は？」と質問
     - 達成目標: ワークフローの非機能要件を明確にする
     - 具体例: 「1日100件」「1回の処理で最大10MB」「5分以内に完了」
     - 確認事項: パフォーマンス要件、セキュリティ要件、コスト制約
  7. n8n-MCP 実行（並列）:

     **実行基準**:
     - **実行タイミング**: 6要素（業務概要、トリガー、データソース、処理内容、出力先、規模と制約）の回答をすべて取得した直後
     - **実行条件**:
       - ユーザーから業務要件の全体像が明確になった時点
       - データソース、処理内容、出力先が具体的に特定できている
       - 類似するワークフローやテンプレートが存在する可能性がある場合
     - **実行内容**:
       ```
       並列実行:
       - search_templates({query: "{{業務概要のキーワード}}", limit: 5})
       - search_nodes({query: "{{データソース名}}", includeExamples: true})
       - search_nodes({query: "{{出力先名}}", includeExamples: true})
       - search_templates_by_metadata({category: "{{業務カテゴリ}}", complexity: "medium"})
       ```
     - **判断ポイント**:
       - テンプレート検索: 業務キーワード（例: "議事録", "データ集計", "通知"）が明確
       - ノード検索: 具体的なサービス名（例: "Google Drive", "Notion", "Discord"）が判明
       - カテゴリ検索: 業務の種類（データ処理、通知、API統合等）が特定可能
     - **スキップ条件**:
       - ユーザーが既に特定のワークフロー構造を持っている場合
       - 完全にカスタムな業務で既存テンプレートが参考にならない場合

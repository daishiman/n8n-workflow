...
- 処理詳細手順:
  ...
  **このステップで達成すべきこと**:
  - step040で設計した接続マトリックスを、n8n互換の完全な設計に変換する
  - 全ノードのn8nノードタイプを100%確定し、デフォルト値に依存しない明示的なパラメータ設定を行う
  - Expression（`{{ $json.fieldName }}`形式）を設計し、ノード間のデータアクセスを定義する
  - 認証情報、エラーハンドリング、タイムゾーン、レート制限を全ノードに設定する
  - 次ステップ（AIエージェント配置）でAI Agent Nodeの詳細設定ができる粒度まで設計を完成させる

  1. **n8n ノードタイプ選定**
     - 達成目標: すべてのノードに対して、正確なn8nノードタイプを確定する
     - 具体例: HTTP Request→n8n-nodes-base.httpRequest、AI処理→@n8n/n8n-nodes-langchain.agent
     - 確認事項: ノードタイプの正確性、バージョン互換性、代替ノードの有無

  2. **データ構造設計（配列形式）**
     - 達成目標: n8nの原則に従い、すべてのデータを配列形式で扱う設計にする
     - 具体例: `[{ "name": "file1.m4a" }]`、`$json["items"]`の配列アクセス
     - 確認事項: データ構造の一貫性、配列変換の必要性、アイテム数の想定

  3. **Expression 設計**
     - 達成目標: ノード間のデータアクセスに使用するExpression（動的な式）を設計する
     - 具体例: `{{ $json.fileId }}`、`{{ $node["Google Drive"].json.items[0].name }}`
     - 確認事項: データパス、存在チェック、デフォルト値、型変換

  4. **認証情報設定**
     - 達成目標: 外部サービス連携に必要な認証情報を環境変数または認証情報ストアで管理する設計にする
     - 具体例: Google Drive→OAuth2認証、Discord→Webhook URL、OpenAI→API Key
     - 確認事項: 認証方式、環境変数名、ハードコード禁止の徹底

  5. **エラーハンドリング設定**
     - 達成目標: 各ノードのエラー時の動作（Continue on Fail、Retry、Error Workflow）を設定する
     - 具体例: API呼び出し→Retry 3回、データ検証→Continue on Fail、致命的エラー→Error Workflow
     - 確認事項: エラーの種類、復旧可能性、ユーザーへの影響

  6. **タイムゾーン設定**
     - 達成目標: すべての日時処理をAsia/Tokyoで統一する
     - 具体例: Schedule Trigger→Asia/Tokyo、日時フォーマット→JST表記
     - 確認事項: タイムゾーンの一貫性、夏時間対応

  7. **レート制限対策**
     - 達成目標: API呼び出しに適切な遅延を設定し、レート制限を回避する
     - 具体例: Discord API（50req/sec）→Wait 20ms、OpenAI（3000req/min）→Wait 20ms
     - 確認事項: API制限値、バッチサイズ、並列実行数

  8. **⚠️ 重要: すべてのパラメータを明示的に設定（デフォルト値に依存しない）**
     - 達成目標: すべてのノードパラメータを明示的に設定し、デフォルト値に依存しない設計にする
  ...
  9. n8n-MCP 実行（並列）:
     ...
     **実行基準**:
     - **実行タイミング**: 全ノードのn8nノードタイプ選定が完了し、パラメータ設定を開始する段階
     - **実行条件**:
       - 各ノードの必須パラメータと推奨設定を確認する必要がある場合
       - Expressionの書き方やデータアクセス方法を確認したい場合
       - 認証設定の詳細を取得する必要がある場合
       - ノード設定の最小要件を検証したい場合
     - **実行内容**:
       ```
       ...
       - search_node_properties({nodeType: "{{各ノードタイプ}}", query: "authentication"})
       - search_node_properties({nodeType: "{{各ノードタイプ}}", query: "error handling"})
       - validate_node_minimal({nodeType: "{{各ノードタイプ}}", config: {{設定案}}})
       ```
     - **判断ポイント**:
       - パラメータ確認: すべてのノードの必須パラメータを取得し、設定漏れを防ぐ
       - 実装例参照: 正しいExpression書き方やパラメータ値の例を確認
       - 認証設定: 各サービスの認証方式と設定方法を確認
       - 最小検証: 各ノード設定が最小要件を満たしているか事前確認
     - **スキップ条件**:
       - すべてのノード設定が完全に確定しており、検証も不要な場合
       - 使用するノードタイプがすべて標準的で、パラメータも明確な場合
